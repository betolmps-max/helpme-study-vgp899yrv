routerAdd(
  ...
  (e) => {
    ...
    if (isNaN(valorBrl) || valorBrl <= 0 || !detalhes) return e.badRequestError('Pedido inválido.')

    $app.runInTransaction((txApp) => {
      const user = txApp.findRecordById('users', e.auth.id)
      const saldo = user.getFloat('saldo') || 0
      if (saldo < valorBrl) throw new BadRequestError('Saldo insuficiente.')

      user.set('saldo', saldo - valorBrl)
      txApp.save(user)
