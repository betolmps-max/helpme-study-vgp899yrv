...
export default function Layout() {
  ...
  const isTermsPage = location.pathname === '/termos-de-uso'
  const hideHeader = isAuthPage || isLanding || isTermsPage

  const [checkingTerms, setCheckingTerms] = useState(true)
  const termsCheckedRef = useRef(false)

  useEffect(() => {
    if (!user || isAuthPage || isLanding || isTermsPage) {
    ...
    pb.collection('termos_uso')
      ...
      .then((res) => {
        if (!isMounted) return
        const termos = res.items[0]
        if (termos) {
          const acceptedAt = user.termos_aceitos_em ? new Date(user.termos_aceitos_em).getTime() : 0
          const updatedAt = new Date(termos.updated).getTime()
          if (acceptedAt < updatedAt) {
      ...
      .finally(() => {
        if (isMounted) setCheckingTerms(false)
      })

    return () => {
      isMounted = false
    }
  }, [user, location.pathname, isAuthPage, isLanding, isTermsPage, navigate])
  ...
  return (
    <div className="flex min-h-screen flex-col bg-slate-50 font-sans">
        ...
        {checkingTerms ? (
          ...
          <div
            className={cn(
              ...
              isAuthPage ? 'max-w-md' : isLanding || isTermsPage ? 'max-w-full' : 'max-w-5xl',
            )}
          >
            <Outlet />
          </div>
      ...
      {!checkingTerms && user && !isAuthPage && !isLanding && !isTermsPage && <TudorTutorial />}
    </div>
  )
}
